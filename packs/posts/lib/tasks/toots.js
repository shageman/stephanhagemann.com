...
const CACHE_FILE_PATH_MINE = '_cache/toots.json'
...
async function writeToCache(cacheFileName, data) {
  const dir = 'src/posts'

  if (!fs.existsSync(dir)) { fs.mkdirSync(dir) }

  await fsPromises.writeFile(cacheFileName, data)
  console.log(`>>> Toots cached to ${cacheFileName}`)
}
...
function createTootPosts() {
  const excerpt = (post) => {
    const content = post.replace(/(<([^>]+)>)/gi, "");
    return content.substr(0, content.lastIndexOf(" ", 80)) + "...";
  };

  console.log('>>> Creating toot posts...');
  const cacheMine = readTootsFromCache(CACHE_FILE_PATH_MINE)
  const scriptFolder = path.dirname(__filename);

  myToots = cacheMine.children.filter((status) => status.in_reply_to_id == null)
  for (let i = 0; i < myToots.length; i++) {
    const startingToot = myToots[i];
    if (startingToot.content == "") { continue; }

    var env = njk.configure([`${scriptFolder}/src/_data/toots`], { autoescape: true });
    Object.keys(filters).forEach(filterName => {
      env.addFilter(filterName, filters[filterName])
    })
    const tootPost = njk.render('list.njk', { 
      toots: [startingToot, startingToot && startingToot.context && startingToot.context.descendants].flat(),
      title: excerpt(startingToot.content).replace('"', "'")
    });
  
    const tootFilename = `src/posts/${startingToot.created_at.split("T")[0]}-toot-${startingToot.created_at.replace(/:|\./g, "")}.md`
    writeToCache(tootFilename, tootPost)
  }
}
...
fetchMyToots(youngestOldTood && youngestOldTood.id).then(
  feed => {
    if (feed) {
      ...
      writeJSONToCache(CACHE_FILE_PATH_MINE, myToots).then(() => {  
        createTootPosts()
      })
    }
  },
